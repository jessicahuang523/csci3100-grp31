...
export const setupFirestoreForNewEvent = async ({
  ...
  if (auth().currentUser && allowedPeople && eventName && eventType && isPublic && location && startingTime) {
    ...
    await addParticipantToEvent({eid: eventDataRef.id, uid, status: "joined"});
    // automatically calls setupFirestoreForNewEventChat()
    const {cid} = await setupFirestoreForNewEventChat({eid: eventDataRef.id, eventName});
...
export const setupFirestoreForNewEventChat = async ({eid, eventName}) => {
  if (auth().currentUser && eid && eventName) {
    ...
    const eventTypeRef = firestore().collection("event_types").where("value", "==", eventType);
...
export const addParticipantToChat = async ({cid, uid}) => {
  if (auth().currentUser && cid && uid) {
    ...
    if (chatDataSnap.exists && userDataSnap.exists) {
      ...
      await chatDataRef.collection("participants").doc(uid).set({username, uid});
      await userDataRef.collection("chats").doc(cid).set({
        cid,
        ...chatDataSnap.data()
      });
    }
...
export const addParticipantToEvent = async ({eid, uid, status}) => {
  if (auth().currentUser && eid && uid && status) {
    ...
    if (eventDataSnap.exists && userDataSnap.exists) {
      ...
      await eventDataRef.collection("participants").doc(uid).set({username, uid, status});
      await userDataRef.collection("events").doc(eid).set({
        eid,
        status,
        ...eventDataSnap.data()
      });
...
export const sendFriendRequest = async ({targetUid}) => {
  if (auth().currentUser && targetUid) {
    ...
    if (userDataSnap.exists && targetDataSnap.exists) {
      const friendRequestSnap = await userDataRef.collection("sent_friend_requests").doc(targetUid).get();
      if (!friendRequestSnap.exists) {
        const time = Date.now();
        {
          const {username, uid} = targetDataSnap.data();
          await userDataRef.collection("sent_friend_requests").doc(targetUid).set({username, uid, time});
        } {
          const {username, uid} = userDataSnap.data();
          await targetDataRef.collection("received_friend_requests").doc(uid).set({username, uid, time});
        }
      }
    }
  }
...
export const acceptFriendRequest = async ({targetUid}) => {
  if (auth().currentUser && targetUid) {
    ...
    if (userDataSnap.exists && targetDataSnap.exists) {
      ...
      {
        const {username, uid} = targetDataSnap.data();
        const friendRequestRef = userDataRef.collection("received_friend_requests").doc(targetUid);
        const friendRequestSnap = await friendRequestRef.get();
        if (friendRequestSnap.exists) {
          await friendRequestRef.delete();
          await userDataRef.collection("friend_list").doc(targetUid).set({username, uid, time});
        }
      } {
        const {username, uid} = userDataSnap.data();
        const friendRequestRef = targetDataRef.collection("sent_friend_requests").doc(uid);
        const friendRequestSnap = await friendRequestRef.get();
        if (friendRequestSnap.exists) {
          await friendRequestRef.delete();
          await targetDataRef.collection("friend_list").doc(uid).set({username, uid, time});
        }
      }
    }
  }
...
export const removeFriendRequest = async ({targetUid}) => {
  if (auth().currentUser && targetUid) {
    ...
    if (userDataSnap.exists && targetDataSnap.exists) {
      {
        const friendRequestRef = userDataRef.collection("sent_friend_requests").doc(targetUid);
        const friendRequestSnap = await friendRequestRef.get();
        if (friendRequestSnap.exists) {
          await friendRequestRef.delete();
        }
        ...
        const friendRequestRef = targetDataRef.collection("received_friend_requests").doc(uid);
        const friendRequestSnap = await friendRequestRef.get();
        if (friendRequestSnap.exists) {
          await friendRequestRef.delete();
        }
...
export const updateProfileData = async ({profileData}) => {
  if (auth().currentUser && profileData && profileData.uid === auth().currentUser.uid) {
    const {uid} = auth().currentUser;
    const userDataRef = firestore().collection("user_profile").doc(uid);
    const userDataSnap = await userDataRef.get();
    if (userDataSnap.exists) {
...
export const uploadProfileImage = async ({imageFile, setUploadProgress}) => {
  if (auth().currentUser && imageFile) {
    const {uid} = auth().currentUser;
    const storageRef = storage().ref("profile_image").child(uid + imageFile.name);
    const uploadTask = storageRef.put(imageFile);
    const progress = (snap) => setUploadProgress(100 * (snap.bytesTransferred / snap.totalBytes));
    const error = (e) => console.log(e.message);
    const complete = async () => {
      const profileImageSrc = await(await uploadTask).ref.getDownloadURL();
      await firestore().collection("user_profile").doc(uid).update({profileImageSrc});
...
export const sendChatMessage = async ({cid, inputMessage, chatParticipants}) => {
  if (inputMessage.length > 0 && auth().currentUser) {
    ...
    if (storedUserData) {
      ...
      firestore().collection("chat").doc(cid).collection("messages").add(toSend);
    }
  }
};
