...
import {addParticipantToEvent, deleteEvent} from "../../utilityfunctions/Utilities";
...
const Event = () => {
  ...
  useEffect(() => {
    if (eid) {
      ...
      const unsubscribeEventParticipantData = eventRef.collection("participants").onSnapshot((snap) => {
        let tmp = [];
  ...
  useEffect(() => {
    if (eventData) {
      const userRef = firestore().collection("user_profile").doc(eventData.hostUid);
      const unsubscribeUserData = userRef.onSnapshot((snap) => setHostUserData(snap.data()));
      return() => {
        unsubscribeUserData();
      };
    }
  ...
  useEffect(() => {
    if (eventData && eventTypeData && gymData) {
      setFoundTypeData(eventTypeData.find((t) => t.value === eventData.eventType));
  ...
  const handleJoinButtonClick = async () => {
    setJoinLoading(true);
    await addParticipantToEvent({eid: eventData.eid, uid: auth().currentUser.uid, status: "joined"});
  ...
  } else if (eventDeleteLoading || !eventData || !eventParticipants || !hostUserData || joinLoading) {
    return <Loading/>;
  } else {
    const {uid} = auth().currentUser;
    return (<div>
      <NavBar/>
      <Jumbotron fluid="fluid" style={{
          padding: "1rem"
        }}>
        <h1>
          ...
          {eventData.eventName}{" "}
          {
            eventParticipants.find((x) => x.uid === uid) && (<Badge pill="pill" color="info">
              Joined
            </Badge>)
          }
        </h1>
      ...
      <div style={{
        ...
        <Row>
          <Col sm={{
            ...
            <Button block="block" outline="outline" tag={Link} to={`/u/${hostUserData.uid}`}>
          ...
          <Col sm={{
            ...
            }}>
            {
              eventParticipants.find((x) => x.uid === uid)
                ? (<Button block="block" tag={Link} to={`/c/${eventData.cid}`}>
                  <i className="fas fa-comment-dots"></i>
                  Chat!
                </Button>)
                : (<Button block="block" onClick={handleJoinButtonClick}>
                  Join
                </Button>)
            }
          </Col>
        ...
        <hr/> {
          uid === hostUserData.uid && (<Row>
            <Col>
              <Button block="block" tag={Link} to="/">
        ...
        }
        {
          uid != hostUserData.uid && (<Row>
            <Col>
              <Button block="block" tag={Link} to="/e">
                <i className="fas fa-undo"></i>
                Back to Events Page
              </Button>
            </Col>
        ...
        <hr/>{
          uid === hostUserData.uid && (<Row>
            <Col>
              <Button block="block" onClick={handleDeleteEventButtonClick}>
