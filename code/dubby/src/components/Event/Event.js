...
const Event = () => {
  ...
  useEffect(() => {
    if (eid) {
      ...
      const unsubscribeEventData = eventRef.onSnapshot((snap) => setEventData(snap.data()));
  ...
  if (userLoading) {
    ...
    return (<div>
      ...
      <Jumbotron fluid="fluid" style={{
        ...
        <h1>
          <i className={foundTypeData.icon}></i>
          [{foundTypeData.display}]{" "}
          {eventData.eventName}{" "}
          {eventParticipants.find((x) => x.uid === uid) && (<Badge color="info" pill="pill">Joined</Badge>)}
        </h1>
        {
      ...
      <div style={{
        ...
        <Row>
          ...
          <Col sm={{
            ...
            <Button type="return" tag={Link} to="/e">Return</Button>
          </Col>
        </Row>
      </div>
    </div>);
