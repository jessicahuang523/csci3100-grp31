...
const AddEvent = () => {
  ...
  const handleEventSubmit = async (e) => {
    ...
    if (startingTime < Date.now()) {
      setErrorMessage("Chill, man. You need to give people some time to prep for this event!");
    } else {
        ...
        <Row>
          ...
          <Col sm={6}>
            <FormGroup>
              ...
              <Input required="required" type="date" id="startingDate" value={eventStartingDate} min={calculateDate()} onChange={(e) => setEventStartingDate(e.target.value)}/>
            </FormGroup>
          </Col>
          <Col sm={6}>
            <FormGroup>
              <Label for="startingTime">Starting Time</Label>
              <Input required="required" type="time" id="startingTime" value={eventStartingTime} onChange={(e) => setEventStartingTime(e.target.value)}/>
            </FormGroup>
          </Col>
        </Row>
        <Row>
          ...
          <Col>
            <Button block="block" color="danger" outline="outline" type="reset" tag={Link} to="/launch">
              Cancel
            </Button>
          </Col>
        </Row>
