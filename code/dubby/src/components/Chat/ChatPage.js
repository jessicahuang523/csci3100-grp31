...
const ChatPage = () => {
  ...
  useEffect(() => {
    if (userData) {
      const {uid} = userData;
      const unsubscribeChatList = firestore().collection("user_profile").doc(uid).collection("chats").onSnapshot((snap) => {
        let tmp = [];
  ...
  if (userLoading || !chatList || !friendListData) {
    ...
    return (<div>
      ...
      <Jumbotron>
        ...
        <Modal isOpen={privateModalOpen} toggle={handlePrivateModalToggle}>
          <Form>
            ...
            <ModalBody>
              <UserList users={friendListData} heading="To..." action={handlePrivateSelectFriend} actionIcon="fas fa-comment-dots" actionColor="primary"/>
            </ModalBody>
            <ModalFooter>
        ...
        <Modal isOpen={groupModalOpen} toggle={handleGroupModalToggle}>
          <Form onSubmit={(e) => {
            ...
            <ModalFooter>
              ...
              <Button color="danger" outline="outline" onClick={handleGroupModalToggle}>
                Cancel
              </Button>
            </ModalFooter>
          </Form>
      ...
      <div style={{
        ...
        {chatList && chatList.length > 0 && chatList.map((chat) => <ChatCard key={chat.cid} cid={chat.cid}/>)}
      </div>
    </div>);
  }
};
