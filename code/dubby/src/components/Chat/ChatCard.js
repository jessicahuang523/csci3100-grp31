...
const ChatCard = ({cid}) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeChatData = chatRef.onSnapshot((snap) => setChatData(snap.data()));
    const unsubscribeChatMessage = chatRef.collection("messages").orderBy("created_at").limitToLast(1).onSnapshot((snap) => snap.forEach((doc) => setChatMessage(doc.data())));
    return() => {
      unsubscribeChatData();
  ...
  useEffect(() => {
    if (chatMessage && chatMessage.sender) {
      ...
      const unsubscribeChatSenderData = userDataRef.onSnapshot((snap) => setChatSenderData(snap.data()));
      return() => {
  ...
  if (!chatData || (chatMessage && !chatSenderData)) {
    ...
    return (<div className="chat-card">
      <Card tag={Link} to={`/c/${cid}`}>
        ...
        <CardBody>
          <CardText>
            {chatSenderData && (<ProfileHead src={chatSenderData.profileImageSrc} size="inline"/>)}
            {
              chatMessage
