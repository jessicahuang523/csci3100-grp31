...
import {sendFriendRequest, updateProfileData, acceptFriendRequest, unfriendFriend, removeFriendRequest} from "../../utilityfunctions/Utilities";
...
const ProfilePage = () => {
  ...
  useEffect(() => {
    if (uid) {
      firestore().collection("user_profile").doc(uid).get().then((s) => {
        ...
        <Row>
          <Col sm={{
            ...
            <Form onSubmit={handleProfileDataSubmit}>
              ...
              <hr/>
              <Input onChange={(e) => handleProfileDataEdit("username", e.target.value)} value={username}/>
              <h2 style={{
                  marginTop: "50px"
                }}>Personal Description</h2>
              <hr/>
              <Input onChange={(e) => handleProfileDataEdit("description", e.target.value)} value={description}/>
              <h2 style={{
                  marginTop: "50px"
                }}>University</h2>
              <hr/>
              <Input onChange={(e) => handleProfileDataEdit("university", e.target.value)} value={university}/>
              <h2 style={{
                  marginTop: "50px"
                }}>Sports</h2>
              <hr/>
              <Input type="select" multiple="multiple" onChange={handleInterestedSportsEdit}>
                {
      ...
      <div style={{
        ...
        <Row>
          <Col sm={{
            ...
            <hr/> {
              interested_sports.length && interested_sports.length > 0 && interested_sports.map !== undefined
                ? (<ul>
                  {
            ...
            <hr/>
            <ProfileActionButton uid={uid} toggleIsEditable={toggleIsEditable} friendListData={friendListData} sentRequestData={sentRequestData} receivedRequestData={receivedRequestData}/>
          </Col>
        </Row>
      </div>
    </div>);
...
const ProfileActionButton = ({uid, toggleIsEditable, friendListData, sentRequestData, receivedRequestData}) => {
  if (uid && uid !== auth().currentUser.uid) {
    if (friendListData && friendListData.find((p) => p.uid === uid)) {
      return (<Button block="block" color="danger" onClick={() => unfriendFriend({targetUid: uid})}>
        Unfriend
      </Button>);
    } else if (sentRequestData && sentRequestData.find((p) => p.uid === uid)) {
      return (<Button block="block" color="warning" onClick={() => removeFriendRequest({targetUid: uid})}>
        Unrequest
      </Button>);
    } else if (receivedRequestData && receivedRequestData.find((p) => p.uid === uid)) {
      return (<Button block="block" color="primary" onClick={() => acceptFriendRequest({targetUid: uid})}>
        Confirm Friend
      </Button>);
    } else {
      return (<Button block="block" color="primary" onClick={() => sendFriendRequest({targetUid: uid})}>
        Add Friend
      </Button>);
    }
  } else 
    return (<Button block="block" onClick={() => toggleIsEditable()}>
      Edit
    </Button>);
  }
;
